...
public abstract class Module {
	...
	//  此处没有定义成Module(String name)的原因是，继承时仍需要写三行代码，并没有简化
	...
	// 类初始化可能造成startup被多次调用，为避免重复初始化，使用started进行标记
	...
	{
		if (shortnames.containsKey(this.name()))
			logger.warn("duplicate module name: " + this.name());
		shortnames.put(this.name(), this.getClass());
	}
	public static Class<? extends Module> findByName(String name) {
		return shortnames.get(name);
	}
	private static final Map<String, Class<? extends Module>> shortnames = new HashMap<String, Class<? extends Module>>();
