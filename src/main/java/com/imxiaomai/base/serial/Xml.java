package com.imxiaomai.base.serial;
...
public final class Xml {
	...
	public Xml add(String tagName, Object value) {
		content.append(f("<%s>%s</%s>", tagName, EscapeUtil.escapeXml(value), tagName));
		return this;
	}
	public Xml attr(String attrName, Object value) {
		if (value != null)
			attrs.append(f(" %s='%s'", attrName, EscapeUtil.escapeXml(value)));
		return this;
	}
	...
	public String toString() {
		return f("<%s%s>%s</%s>", tag, attrs, content, tag);
	}
	public String toDoc(String charset) {
		return f(XML_DECLARATION, charset) + this.toString();
	}
	public String toDoc() {
		return toDoc("GBK");
	}
	...
	public static String textNotNull(Element elem, String xpath) {
		return trimToEmpty(text(elem, xpath));
	}
	...
	public static final class ParsedXml {
		...
		public List<Element> elems(String xpath) {
			List<Element> src = new ArrayList<>(asList(this.root));
	...
	private static void testParse(String xml) throws DocumentException, IOException, XmlPullParserException {
		...
		p(doc.elem("a"));
		p(doc.text("b/b1"));
		p(doc.text("b/b1/@attr"));

		Element e = doc.elem("b/b1");
		p(Xml.text(e, "@attr"));
		p(Xml.textNotNull(e, "@attr"));
		p(Xml.text(e, "@non_exist"));
		p(Xml.textNotNull(e, "@non_exist"));
	}
