...
public class ProxoolRunner extends QueryRunner {
	...
	private static final AutoLog mysqlAll = AutoLog.of("mysql.all");

	private static final AutoLog mysqlUpdate = AutoLog.of("mysql.update");

	private static final AutoLog mysqlSelect = AutoLog.of("mysql.select");
	...
	public static void registerDB(String alias, String driverUrl, String user, String password, int maxConn) {
		...
		try {
			...
			String jdbcUri = "proxool.%s:%s:jdbc:mysql://%s?zeroDateTimeBehavior=convertToNull&useUnicode=true&characterEncoding=utf-8&autoReconnect=true";
    ...
    public Map<Long, Map<Long, String>> queryKeyeds(String sql, Object... params) throws SQLException {
        return query(sql, keysHandler, params);
    }
    static final ResultSetHandler<Map<Long, Map<Long, String>>> keysHandler = new AbstractMapsHandler<Long, Long, String>() {
        @Override
        protected Long createKey(ResultSet resultSet) throws SQLException {
            return resultSet.getLong(1);
        }

        @Override
        protected Long createSubKey(ResultSet resultSet) throws SQLException {
            return resultSet.getLong(2);
        }

        @Override
        protected String createSubValue(ResultSet resultSet) throws SQLException {
